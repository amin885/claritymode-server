...
router.get('/status', async (req, res) => {
  try {
    ...
    res.json({
      ...
      connectors: { vidiq: { connected: Boolean(connector.rows[0]), metadata: connector.rows[0]?.metadata || {}, updatedAt: connector.rows[0]?.updated_at } },
      profiles: { [assignments.SKILL_ID]: { value: profileResult.rows[0]?.profile || {}, updatedAt: profileResult.rows[0]?.updated_at || null } },
    })
  } catch {
...
router.put('/profiles/youtube-script-producer', async (req, res) => {
  try {
    ...
    if (!profile.channelPurpose || !profile.idealViewer || !profile.viewerProblems) {
      return res.status(400).json({ error: 'Complete the channel purpose, ideal viewer, and viewer problems.' })
    }
    await db.query(
...
router.put('/connectors/vidiq', async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [req.user.sub, credentials.encrypt(apiKey), { toolCount: validation.toolCount }],
    )
    res.json({ ok: true, connected: true, metadata: { toolCount: validation.toolCount } })
  } catch (error) {
